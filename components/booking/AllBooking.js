...
function AllBooking() {
    ...
    const setBookings = () => {
        ...
        allBooking && allBooking.forEach(booking => {
            data.rows.push({
                id: booking.id,
                checkIn: new Date(booking.checkInDate).toLocaleString('en-US'),
                checkOut: new Date(booking.checkOutDate).toLocaleString('en-US'),
                amount: `$${booking.room.pricePerNight * booking.daysOfStay}`,
                actions:
                    <>
                        <Link href={`/booking/single/${booking.id}`}>
    ...
    const downloadInvoice = async (booking) => {
            ...
            "products": [
                {
                    ...
                    "description": `${booking.room_id}`,
                    "tax": 0,
                    "price": booking.payment_id
                }
            ],
        ...
        easyinvoice.download(`invoice_${booking.id}.pdf`, result.pdf)
